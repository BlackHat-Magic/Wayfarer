...
def makeRuleset(request, ruleset, instruction):
    if(current_user.id != ruleset.userid and (not ruleset.is_shareable or user in ruleset.editors or user in ruleset.viewers)):
        flash("You must have permission to view a ruleset to duplicate it.", "red")
    elif(instruction == "duplicate" and (ruleset.is_shareable or user in ruleset.editors or user in ruleset.viewers)):
        if(request.form.get("same_viewers")):
        ...
        if(request.form.get("same_editors")):
            ...
            editors = []
        if(request.form.get("is_shareable")):
            is_shareable = True
        else:
            is_shareable = False
        new_ruleset = Ruleset(
        ...
        # create blank new ruleset. also update duplication for custom identifier.
        pass