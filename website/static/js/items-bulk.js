document.addEventListener ("alpine:init", () => {
    Alpine.data ("main", () => ({
        ...
        addremtaglist: [],
        appendAddRemTag() {
            if(!(this.addremtaglist.includes(document.querySelector("#addremtags").value))){
                this.addremtaglist.push(document.querySelector("#addremtags").value);
            }
        },
        removeAddRemTag(index) {
            output = [];
            for (let i = this.addremtaglist.length -1; i >=0; i--) {
                if(i != index) {
                    output.push(this.addremtaglist[i]);
                }
            }
            newoutput = [];
            for (let i = output.length - 1; i >= 0; i--) {
